...
## What is an Agent
An Agent is a system centered around an LLM, integrating tool invocation, planning capabilities, and memory functions. It can invoke tools and complete tasks based on user instructions. Many derivative concepts have emerged from Agents, such as skills, harness engineering, and Agentic Search. To understand these, I studied the leaked code of Claude Code and the currently popular speckit (harness framework) and conducted an analysis. Next, I will implement an Agent that integrates these features from scratch and explain its working principles.

## Implementation of the Core Agent Logic
To build a good Agent, you must understand that the core of an Agent is a while loop (called SAM LOOP in Claude Code). This loop continuously thinks -> makes decisions and selects tools -> executes and observes -> self-corrects and terminates until the task is completed. During this process, memory can also be saved for future use.
```python
    def start_loop(self):
        ...
        while True:
            try:
                ...
                break
```
* The Chat Completions API (also often referred to as Chat format or OpenAI Message format) is used here. This interface specification was first released by OpenAI with GPT-3.5-Turbo. Due to OpenAI's industry leadership, this specification has become the de facto standard for large model communication. It mainly defines three roles: user, assistant, and system. Code for detecting keyboard interrupts and exits is also added here, allowing users to better terminate the Agent when needed.

In this loop, the system prompt and user input are first added to the message list, then the _process_turn() method is called to handle the logic of this turn. This method calls the LLM to generate a response, parses tool calls in the response, executes the tools and obtains results, and finally feeds the results back to the LLM for the next round of thinking and decision-making.
```python
...
```
* The Agent uses the XML protocol (the core interaction protocol of Claude Code), distinguishing content through specific tags so that the model can clearly know how to organize the output, and it is also convenient for us to parse and execute. I will use the Code Interpreter as the underlying core to build a layer of "logical isolation wall". Instead of directly letting the model operate the system terminal (CMD/Bash), tasks are executed through the Python environment. This avoids issues with special characters on the terminal, allows better use of sandboxing to control security and execution environment, and also leverages Python's rich libraries to accomplish various tasks.
  
This code handles the core logic of the Agent, which is also the heart of the entire Agent. It is mainly responsible for calling the large model to generate a response, parsing tool calls in the response, executing the tools and obtaining results, and finally feeding the results back to the LLM for the next round of thinking and decision-making. In this method, a variable for the maximum number of retries is defined to prevent the model from continuously outputting content that does not conform to the protocol, leading to an infinite loop. Then it enters a while loop, each time checking whether the length of the message list exceeds the preset compression threshold. If it does, the _condense_context() method is called for context compression. The thinking and finish tags are used for the model's thinking process and task completion summary, respectively, while the python tag is used for the model to output Python code that needs to be executed. The Python code runs in a sandbox to prevent security issues, and the execution results are fed back to the model for the next round of thinking and decision-making.
```python
...
```
* A human-in-the-loop authorization mechanism is implemented here. Whenever the model outputs Python code that needs to be executed, it first requests user approval. The user can choose to approve, reject, or enable automatic approval mode. This ensures security while providing some flexibility. At the same time, the code is written to a temporary file and executed in a sandbox via the subprocess module. The execution results are captured and fed back to the model for the next round of thinking and decision-making. The execution file is deleted after completion to reduce the burden of manual deletion for the user.

The main logic code of the Agent is the above code plus some detailed handling, such as: processing the user's approval result, handling timeout exceptions, handling abnormal situations, etc. However, how to make the Agent better understand the task, how to make the Agent better think and make decisions, and how to design system prompts to guide the Agent's behavior are all very important details and key factors for the success of the Agent. Next, I will introduce some of these detailed handling parts.

## Implementation of Agent Prompts
The design of Agent prompts is very important, directly affecting the Agent's performance and capabilities. A good prompt can guide the Agent to better understand the task, better think and make decisions, and thus better complete the task. Therefore, we need an Agent identity and constraint file to describe the Agent's identity and behavioral guidelines.

### Agent Identity and Behavioral Guidelines File
```txt
...
```
* This prompt file defines the Agent's identity and behavioral guidelines, as well as the Agent's thinking process and memory strategy. Through such prompt design, the Agent can be guided to better understand the task, better think and make decisions, and thus better complete the task. At the same time, the prompt also constrains the Agent's behavior, preventing the Agent from outputting unexpected content or performing unsafe operations. Using a prompt file in md format not only makes it convenient for us to parse and process when needed, but also helps the Agent better understand and use these prompts. This prompt also emphasizes that the core of the Agent is the code interpreter, completing tasks through code rather than directly operating the system terminal, which can better handle complex tasks and avoid problems caused by special characters.

### Python Code Specification File
```txt
    ...
    * **静默启动程序**：
    ...
    ```
```
* This Python code specification file defines the programming standards that the model must follow when outputting Python code that needs to be executed, ensuring 100% operation in the local Windows environment. Through such specification design, the model can be guided to output code that better meets expectations, avoiding execution failures caused by path or encoding issues. At the same time, the specification also constrains the model's behavior, preventing the model from outputting unsafe or unexpected Python code. This specification file also provides code snippets for common tasks, so that the model can reference and use these code snippets to complete tasks when needed.

### Skill File
The Agent's skill file is used to store the skills the Agent has learned. Through this file, the Agent can persist the learned skills or read skill information from the file for subsequent use, executing according to the skills and methods provided in the skills.
```txt
...
Based on the harness engineering approach proposed in specKit, I designed a harness constraint file that follows the process: generate constraints -> clarify requirements -> formulate a plan -> break down tasks -> start implementation, to guide the Agent in better thinking and decision-making.
```txt
...
The following method can be used to import tags and the above system prompt files into the Agent, so that the Agent can better understand and use these prompts to guide its behavior.
```python
...
To prevent the Agent from having insufficient model context due to excessive context during execution, we need to implement a compression capability. I implemented compression based on the clone Agent mentioned in a paper by Anthropic.
```python
...
```
* This method compresses the context into a snapshot when the message queue becomes too long, adds it as a system prompt to the message list, and retains the last user input or environment feedback as a message to trigger continued execution. In this way, the Agent can continue executing tasks when the context is too long without the model being unable to process due to excessive context. At the same time, the prompt guides the Agent to understand the current state snapshot, enabling better thinking and decision-making.


## Other Details
### Color Palette
Different colors can be used to distinguish outputs from different roles, enhancing user experience and readability.
```python
class Colors:
    CYAN = '\033[96m'
    GREEN = '\033[92m'
...
* This method uses regular expressions to extract the content of the thinking, python, and finish tags from the XML response returned by the model.
