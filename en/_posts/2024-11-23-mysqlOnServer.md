...
Before setting up, you need to have a virtual machine or server deployed and possess some basic Linux knowledge.
...
1. Deploy the MySQL repository command:
```bash
cd /opt
...
```
Since `MariaDB` is forked from the `MySQL` source code branch, it conflicts with `MySQL`. If a conflict error with `mariadb-libs` occurs, execute the following code:
```bash
...
After logging in, you need to change the password before proceeding. Execute the following code:
```sql
...
### Permission Errors
If you modify `MySQL` permissions incorrectly and cannot log in to `MySQL`, you can execute the following command to skip permission checks
