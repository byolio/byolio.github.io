---
layout: post
title: How RocketMQ's NameSrv Works
...
> This article aims to introduce how RocketMQ's NameSrv works.
...
Producers obtain routing information about Topics from NameSrv every 30 seconds, then establish a long connection with the corresponding Broker, and cache the routing information in memory (which will be lost upon restart). Subsequently, they directly send messages to the corresponding Topic. \
...
Because Rocket's NameSrv implementation is very lightweight, NameSrv instances within the cluster do not exchange any information with each other. Therefore, Brokers need to report routing information to every NameSrv in the cluster, so that each NameSrv stores complete routing information. \
