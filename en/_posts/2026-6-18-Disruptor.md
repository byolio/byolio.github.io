---
layout: post
title: Introduction and Usage of Disruptor Framework
...
> 🌐 [中文](/2026-6-18-Disruptor/) | [English](/en/en/_posts/2026-6-18-Disruptor/)

## Introduction
When building multi-player team collaboration or real-time chat systems using WebSocket, a performance bottleneck often arises: when a WebSocket thread receives a team message, if it directly processes time-consuming business logic synchronously in the current thread (such as database persistence, permission verification, broadcast distribution, etc.), the thread will be occupied for a long time.

In high-concurrency scenarios, this synchronous processing method quickly exhausts the network thread pool, causing subsequent team messages to be blocked, and even leading to connection timeouts and message loss, resulting in a cliff-like drop in overall system throughput.
...
## Disruptor Overview
Disruptor is a high-performance, low-latency **in-memory concurrency framework** developed by LMAX, a British foreign exchange trading company. It was originally designed to meet the low-latency and high-throughput requirements of financial trading systems in multi-threaded environments.
...
1. **Severe Lock Contention**: Traditional blocking queues typically rely on heavy locks like `ReentrantLock` or `synchronized` to ensure thread safety during enqueue (Put) and dequeue (Take). Under high concurrency, the overhead of thread context switching is enormous.
2. **False Sharing Problem**: In traditional linked list or array structures, the head and tail pointers of the queue are often very close in memory. When multi-core CPUs read them, they are loaded into the same cache line, causing frequent cache invalidation between cores, triggering write-backs to main memory, and severely slowing down CPU efficiency.
3. **Garbage Collection (GC) Pressure**: Frequent enqueue and dequeue operations involve the creation and destruction of a large number of objects, easily causing the JVM to trigger GC frequently, leading to system pauses (Stop-The-World).

### 2. Core Black Technology of Disruptor
To squeeze the last drop of CPU performance, Disruptor introduces the following core designs:
...
#### ② Unique Lock-Free Design: Sequence
Disruptor does not use traditional head and tail pointers; instead, each producer and consumer maintains its own `Sequence` (a sequence number, essentially an incrementing `long` value).
...
In the underlying implementation of core components (such as `Sequence`), Disruptor artificially pads 7 useless `long` variables before and after the core variable. This ensures that the core sequence number **exclusively occupies a 64-byte CPU cache line**, completely avoiding the false sharing problem and maximizing the cache hit rate of multi-core CPUs.


## How to Import Disruptor Framework in SpringBoot
...
The Event is the data carrier flowing through the RingBuffer. Disruptor pre-instantiates these Events at startup.
```java
@Data
...
The consumer retrieves data from the RingBuffer and executes the specific time-consuming business logic. Here we use WorkHandler, which supports multiple consumer threads working together, and each message is processed by only one consumer (work queue mode).
```java
@Component
@Slf4j
public class TeamMessageWorkHandler implements WorkHandler<TeamMessageEvent> {
    ...
    public void onEvent(TeamMessageEvent event) throws Exception {
        ...
        String response = "{\"status\":\"success\",\"data\":\"Processing completed\"}";
...
The producer is responsible for delivering the high-concurrency messages received by WebSocket into the Disruptor's RingBuffer. The delivery process is lock-free and extremely fast.
```java
@Component
public class TeamMessageProducer {
    ...
    public void publishEvent(String payload, WebSocketSession session, String teamId, String userId) {
        ...
        try {
            // 2. Get the pre-allocated Event object corresponding to the sequence number and fill its properties
            ...
            // 3. Publish the event (must be published in finally to ensure the sequence number is submitted normally even if an exception occurs)
...
```
1. Integrate in the WebSocket Handler

Finally, in the WebSocket network event handler, we no longer process business synchronously; instead, we immediately throw the message to the Producer, then release the WebSocket thread to handle the next network request.
```java
@Component
@Slf4j
public class GameTeamWebSocketHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        // Directly "deliver instantly" the high-concurrency network message to the asynchronous queue, without any time-consuming processing here
...
In a production environment, distributed systems are constantly facing version iterations, service restarts, or scaling down. If we forcefully terminate the process when closing the Spring container, it may lead to the following disastrous consequences:

1. Data loss: At this time, there may still be backlogged team messages in the RingBuffer that have not been processed by consumers. Forceful shutdown will cause this data to disappear.

2. State inconsistency: The consumer thread may be in a critical state halfway through execution (e.g., the database has just finished writing to table A but has not yet updated table B). Direct interruption will cause dirty states in business data.
...
In high-concurrency environments, WebSocket threads only need to perform two extremely fast operations: "receive message -> insert into RingBuffer", reducing the single processing time from hundreds of milliseconds to nanoseconds. The underlying lock-free ring array and memory barrier black technology ensure the safety and high performance of multi-threaded message delivery, perfectly solving the message blocking problem in multi-player team collaboration systems.
