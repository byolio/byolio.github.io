...
```
Note: /opt/hadoopfilename is the directory where you extracted Hadoop 3.x. Replace it with your Hadoop extraction directory (the previous /dest/), such as /opt/hadoop-3.4.0.
Start Hadoop:
```bash
source /etc/profile
```
...
Please enable your VPN/proxy and reopen the official website.
...
```
Note: /opt/hadoopfilename is the directory where you extracted Hadoop 3.x. Replace it with your Hadoop extraction directory (the previous /dest/), such as /opt/hadoop-3.4.0.
Then, in the file, below the following lines:
```bash
