...
> 🌐 [中文](https://www.byolio.blog/2024-12-14-PyTorchModel/) | [English](https://www.byolio.blog/en/en/_posts/2024-12-14-PyTorchModel/)
...
You can refer to my previous [article](https://byolio.top/2024/11/30/torch/), I won't spend time elaborating here.
...
```
### 3. torch.optim
torch.optim is the optimizer module for neural networks in PyTorch. It provides many classes and functions for optimizing neural networks for backpropagation, including Adam optimizer, SGD optimizer, etc.
```python
...
### Activation Layer:
#### nn.ReLU
nn.ReLU is a class in PyTorch used to define the ReLU activation function. It accepts the input as a parameter and internally computes the value of the ReLU function. During forward propagation, it performs the ReLU operation on the input and then applies a nonlinear transformation through the activation function.
```python
relu = nn.ReLU(inplace=False)
```
Parameters:
* inplace: Whether to directly replace the input tensor's value (default is False)
#### nn.Softmax
nn.Softmax is a class in PyTorch used to define the Softmax activation function. It accepts the input as a parameter and internally computes the value of the Softmax function. During forward propagation, it performs the Softmax operation on the input and then applies a nonlinear transformation through the activation function.
```python
softmax = nn.Softmax(dim=None)
```
Parameters:
* dim: Dimension along which Softmax is computed (default is None)
#### nn.Sigmoid
nn.Sigmoid is a class in PyTorch used to define the Sigmoid activation function. It accepts the input as a parameter and internally computes the value of the Sigmoid function. During forward propagation, it performs the Sigmoid operation on the input and then applies a nonlinear transformation through the activation function.
```python
sigmoid = nn.Sigmoid()
```
### Pooling Layer:
#### nn.MaxPool2d
nn.MaxPool2d is a class in PyTorch used to define a max pooling layer. It accepts parameters such as pooling window size, stride, and padding, and internally computes the max pooling operation. During forward propagation, it performs the pooling operation on the input and then downsamples through the pooling operation.
```python
...
#### nn.BatchNorm2d (Set the model to training mode during training, and to evaluation mode during testing)
nn.BatchNorm2d is a class in PyTorch used to define a batch normalization layer. It accepts the number of input channels as a parameter and internally computes the batch normalization operation. During forward propagation, it performs the batch normalization operation on the input and then normalizes through the batch normalization operation.
```python
...
### Flatten Layer:
#### nn.Flatten
nn.Flatten is a class in PyTorch used to define a flatten layer. It accepts the dimensions of the input as parameters and internally flattens the input into a one-dimensional vector. During forward propagation, it flattens the input into a one-dimensional vector and then downsamples through the flatten operation.
```python
flatten = nn.Flatten(start_dim=1, end_dim=-1)
```
Parameters:
* start_dim: Starting dimension for flattening
* end_dim: Ending dimension for flattening (default is -1), flattens to the last dimension
### Loss Function:
#### nn.CrossEntropyLoss
nn.CrossEntropyLoss is a class in PyTorch used to define the cross-entropy loss function. It accepts the input and target labels as parameters and internally computes the cross-entropy loss. During forward propagation, it performs the loss calculation on the input and target labels through the loss function.
```python
...
#### nn.MSELoss
nn.MSELoss is a class in PyTorch used to define the mean squared error loss function. It accepts the input and target labels as parameters and internally computes the mean squared error loss. During forward propagation, it performs the loss calculation on the input and target labels through the loss function.
```python
...
1. Parts defined in the subclass of nn.Module and their order:
    1. Repeatable parts (you can remove unnecessary layers, adjust parameters accordingly): \
        Convolutional Layer + Activation Layer + Pooling Layer + Batch Normalization Layer
    2. Non-repeatable parts:
...
2. Parts not defined in the subclass of nn.Module: \
    ...
    optimizer = torch.optim.SGD(model.parameters(), lr=0.01, momentum=0.9)
    ```
    2. Usage (used after calling the forward propagation function):
    ```python
    # Loss function
    ...
    optimizer.step()
    ```
## Example (this example indicates the order of model construction; the Modules may vary when training different models):
